...
import { AddBatchRequest, AddBatchResponse, Product } from "./interfaces";
import { ulid } from "ulid";
import { Batch } from "../getBatch/interfaces";
...
const addBatch = async (
  ...
  try {
    ...
    if (!dt_fabricacao || isNaN(new(dt_fabricacao).getTime())) {
      res
    ...
    if (!dt_validade || isNaN(new(dt_validade).getTime())) {
      res
        .status(StatusCodes.INTERNAL_SERVER_ERROR)
        .json(
          Status.error(
